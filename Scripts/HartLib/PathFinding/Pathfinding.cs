    ...
    public class PathFinding<TBlock>
    {
        ...
        public List<PathFindingCell<TBlock>> FindPath(Vector2i startPos, Vector2i endPos, HashSet<TBlock> blockingTiles, int collider_layer = 0, bool include_checking_last = true, bool diagonals = false, bool big = false)
        {
            ...
            if (big)
            {
                ...
                {
                    if (CheckIfInRange(pos, gridSize) is false || grid[pos.x, pos.y] is null || grid[pos.x, pos.y].CheckForTiles(blockingTiles, checkBlockingTiles) || grid[pos.x, pos.y].CheckForColliders(collider_layer, checkForColliders))
                    {
                        return null;
                    }
                }
            ...
            while (openSet.Count > 0 && safeCheck > 0)
            {
                ...
                {
                    if (include_checking_last && (currentCell.CheckForTiles(blockingTiles, checkBlockingTiles) || currentCell.CheckForColliders(collider_layer, checkForColliders)))
                    {
                        return null;
                    }
                ...
                {
                    if (closedSet.Contains(neigbours[i]) || ((neigbours[i].CheckForTiles(blockingTiles, checkBlockingTiles) || (neigbours[i].CheckForColliders(collider_layer, checkForColliders))) && neigbours[i] != endCell))
                    {
                        continue;
                    }
                    ...
                    if (big && neigbhourPos != endPos)
                    {
                        ...
                        foreach (var pos in adjacent)
                        {
                            ...
                            if (closedSet.Contains(adjacentNeigbour) || ((adjacentNeigbour.CheckForTiles(blockingTiles, checkBlockingTiles) || (adjacentNeigbour.CheckForColliders(collider_layer, checkForColliders))) && adjacentNeigbour != endCell))
                            {
                                adjacentCheck = false;
                                break;
                            }
        ...
        public PathFinding(Vector2i _gridSize, Func<Vector2i, HashSet<TBlock>, bool> _checkBlockingTiles, Func<Vector2i, int, bool> _checkForColliders = null)
        {
            gridSize = _gridSize;
